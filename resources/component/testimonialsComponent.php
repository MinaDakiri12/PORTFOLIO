    ...
    if (isset($_POST['submit'])) {
        try {
          if(empty($_FILES['profile']['name'])){
            ...
            $sql = "UPDATE `testimonials` SET `full_name` = ?, `content` = ?, `role` = ?,`profile` = ? WHERE `testimonials`.`id` = ?";
            $update_testimonials = $pdo->prepare($sql);
            $update_testimonials->execute([$_POST['full_name'], $_POST['content'], $_POST['role'], $cover_id, $_POST['testimonials_id']]);
